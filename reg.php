...
function sendpassemail($emailaddress, $password, $username) {
	...
	$parse['gameurl']  = $game_config['game_name'];
	$parse['user'] = $username;
	$parse['password'] = $password;
	$email             .= parsetemplate($lang['mail_welcome'], $parse);
	$mailtitle = parsetemplate($lang['mail_title'], $parse);
	$status            = mymail($emailaddress, $mailtitle, $email);
	return $status;
}
...
if ($_POST) {
	...
	if ($errors != 0) {
		...
		if (sendpassemail($_POST['email'], "$newpass", $_POST['character'])) {
			$Message .=  htmlentities($_POST["email"]);
		} else {
			$Message .= " (" . htmlentities($_POST["email"]) . ")";
			$Message .= "<br><br>". $lang['error_mailsend'] ." <b>" . $newpass . "</b>";
		}
		$Message .= $lang['reg_return_main'];
